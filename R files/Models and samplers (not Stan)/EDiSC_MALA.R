...
#Write a function to calculate the gradient of the log likelihood w.r.t. xi for a single time slice t and a single sense k
...
get.chi.MALA.gradient = function(chi, psi.tilde, chi.prior.mean, chi.prior.var, sense.probs, rho.times.word.counts, snippet.lengths) {
...
# 
#   L.P.rho.psi.tilde = sapply(1:num.senses, function(l){t(rho) %*% psi.tilde[,l,snippets$Time] %*% (snippet.lengths*P[,l])})
#   
#   - (chi - chi.prior.mean) / chi.prior.var +
#     as.matrix(rho.times.word.counts %*% P) - L.P.rho.psi.tilde
# 
# }#get.chi.MALA.gradient


#Write a function to calculate the MALA gradient for theta updates for a single time slice
get.theta.MALA.gradient = function(theta, psi.tilde, theta.prior.mean, theta.prior.var, sense.probs, rho.times.word.counts, snippet.lengths) {
  
  prior.grad = - (theta - theta.prior.mean) / theta.prior.var
  
  lik.grad = foreach(l = 1:num.senses, .combine = '+') %do% {
    get.xi.k.t.gradient(l, psi.tilde[,l], sense.probs, rho.times.word.counts, snippet.lengths)
  }
...
#       rowSums( rho.times.word.counts - 
#                  t(snippet.lengths * ((t(sense.probs) / colSums(sense.probs)) %*% t(psi.tilde) %*% rho)))
#   )
#   
# }#get.theta.MALA.gradient


#Write a function to calculate the MALA gradient for sigma updates
get.sigma.MALA.gradient = function(sigma, psi.tilde, sigma.prior.mean, sigma.prior.var, sense.probs, 
...
#Write a function to calcualte the log MH ratio given current and proposed phi, theta and chi
...
save(num.iterations, N, kappa.phi, kappa.chi, kappa.theta, kappa.sigma, alpha.phi, alpha.theta, random.seed, file = "other.parameters.RData")
...
for (b in 1:num.runs) {
  ...
  save(phi.MALA.prop.scale, chi.MALA.prop.scale, theta.MALA.prop.scale, sigma.MALA.prop.scale, 
       accept.count.phi, accept.count.chi, accept.count.theta, accept.count.sigma, phi.jumps, chi.jumps, 
       theta.jumps, sigma.jumps, file = paste("MALA.parameters.RData", filename.suffix, "RData", sep = "."))
...
accept.count.sigma / num.iterations
...
plot(as.mcmc(sigma.sim[(burn.in+1):num.samples,Words]))
...
load(paste("sigma.sim", filename.suffix, "RData", sep = "."))
