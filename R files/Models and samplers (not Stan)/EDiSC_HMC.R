...
get.phi.g.t.gradient = function(phi.g.t, phi.g.t.tilde, phi.g.t.prior.mean, phi.t.prior.var, 
  ...
  - (phi.g.t - phi.g.t.prior.mean) / phi.t.prior.var - snippet.counts.g.t * phi.g.t.tilde +
    colSums(t(sense.probs.g.t) / colSums(sense.probs.g.t))
...
get.chi.gradient = function(chi, psi.tilde, chi.prior.mean, chi.prior.var, sense.probs, rho.times.word.counts, snippet.lengths) {
  
  prior.grad = - (chi - chi.prior.mean) / chi.prior.var
  
  lik.grad = foreach(l = 1:num.senses, .combine = 'cbind') %do% {
...
get.theta.t.gradient = function(theta.t, psi.t.tilde, theta.t.prior.mean, theta.t.prior.var, sense.probs.t, 
                                rho.times.word.counts.t, snippet.lengths.t) {
  
  prior.grad = - (theta.t - theta.t.prior.mean) / theta.t.prior.var
  
  lik.grad = foreach(l = 1:num.senses, .combine = '+') %do% {
...
get.sigma.gradient = function(sigma, psi.tilde, sigma.prior.mean, sigma.prior.var, sense.probs, 
  ...
  lik.grad = word.counts.totals - foreach(t = 1:num.periods, .combine = '+') %do% {
...
propose.new.phi.g.t.HMC = function(num.steps, epsilon, phi.g.t, phi.g.t.tilde, psi.t.tilde, phi.g.t.prior.mean, 
                                   phi.t.prior.var, sense.probs.g.t, snippet.counts.g.t){
...
propose.new.chi.HMC = function(num.steps, epsilon, chi, psi.tilde, chi.prior.mean, chi.prior.var, 
                               sense.probs, rho.times.word.counts, snippet.lengths){
...
propose.new.theta.t.HMC = function(num.steps, epsilon, theta.t, psi.t.tilde, theta.t.prior.mean, theta.t.prior.var, 
                                   sense.probs.t, rho.times.word.counts.t, snippet.lengths.t){
...
propose.new.sigma.HMC = function(num.steps, epsilon, sigma, psi.tilde, sigma.prior.mean, sigma.prior.var, 
                                 sense.probs, word.counts.totals, snippet.lengths){
...
for (b in 1:num.runs) {
  ...
  for (i in 1:num.iterations) {
    ...
    chi.HMC.proposal = propose.new.chi.HMC(num.leapfrog.steps.chi, sqrt(chi.prop.scale), chi, psi.tilde, chi.prior.mean, 
                                           kappa.chi, sense.probs, rho.times.word.counts, snippet.lengths)
