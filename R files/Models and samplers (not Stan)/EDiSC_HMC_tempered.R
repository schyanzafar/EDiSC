...
target.accept.rate.sigma = 0.651
...
chi.prop.scale.init = 2.4^2 / (embedding.dim * num.senses)^2 / num.leapfrog.steps.chi
theta.prop.scale.init = 2.4^2 / embedding.dim^2 / num.leapfrog.steps.theta
sigma.prop.scale.init = 2.4^2 / num.words / num.leapfrog.steps.sigma
...
#Function to calculate the gradient of the log likelihood w.r.t. xi for a single time slice t and a single sense k
get.xi.k.t.gradient = function(k, psi.k.t.tilde, sense.probs.t, rho.times.word.counts.t, snippet.lengths.t) {
  
  as.numeric( ( rho.times.word.counts.t - outer(colSums(rho * psi.k.t.tilde), snippet.lengths.t)) %*% 
                (sense.probs.t[k,] / colSums(sense.probs.t)) )
  
}#get.xi.k.t.gradient


#Function to calculate the log posterior gradient wrt chi for all senses
get.chi.gradient = function(chi, psi.tilde, chi.prior.mean, chi.prior.var, sense.probs, 
                            rho.times.word.counts, snippet.lengths, temper = FALSE, inv.temp = 1) {
  
  prior.grad = - (chi - chi.prior.mean) / chi.prior.var
  
  lik.grad = ifelse(temper, inv.temp, 1) * foreach(l = 1:num.senses, .combine = 'cbind') %do% {
    foreach(t = 1:num.periods, .combine = '+') %do% {
      get.xi.k.t.gradient(l, psi.tilde[,l,t], sense.probs[,SnippetIDs.by.time[[t]], drop=FALSE], 
                          rho.times.word.counts[,SnippetIDs.by.time[[t]]], snippet.lengths[SnippetIDs.by.time[[t]]])
    }
  }
...
get.theta.t.gradient = function(theta.t, psi.t.tilde, theta.t.prior.mean, theta.t.prior.var, sense.probs.t, 
                                rho.times.word.counts.t, snippet.lengths.t, temper = FALSE, inv.temp = 1) {
  
  prior.grad = - (theta.t - theta.t.prior.mean) / theta.t.prior.var
  
  lik.grad = ifelse(temper, inv.temp, 1) * foreach(l = 1:num.senses, .combine = '+') %do% {
    get.xi.k.t.gradient(l, psi.t.tilde[,l], sense.probs.t, rho.times.word.counts.t, snippet.lengths.t)
  }
...
#Function to calculate the log posterior gradient wrt sigma
get.sigma.gradient = function(sigma, psi.tilde, sigma.prior.mean, sigma.prior.var, sense.probs, 
                              word.counts.totals, snippet.lengths, temper = FALSE, inv.temp = 1) {
  
  prior.grad = - (sigma - sigma.prior.mean) / sigma.prior.var 
  
  P = t(sense.probs)/colSums(sense.probs)
  
  lik.grad = ifelse(temper, inv.temp, 1) * (word.counts.totals - foreach(t = 1:num.periods, .combine = '+') %do% {
    colSums(snippet.lengths[SnippetIDs.by.time[[t]]] * (P[SnippetIDs.by.time[[t]],] %*% t(psi.tilde[,,t])))})
...
#Function adapted from Neal, R.M., 2011. MCMC using Hamiltonian dynamics
propose.new.chi.HMC = function(num.steps, epsilon, chi, psi.tilde, chi.prior.mean, chi.prior.var, 
                               sense.probs, rho.times.word.counts, snippet.lengths, temper = FALSE, inv.temp = 1){
  
  #Generate initial momentum vector
  p = rnorm(embedding.dim*num.senses)
  ...
  grad_U = -get.chi.gradient(chi, psi.tilde, chi.prior.mean, chi.prior.var, sense.probs,
                             rho.times.word.counts, snippet.lengths, temper, inv.temp)
  p = p - epsilon * grad_U / 2
  ...
  for (s in 1:num.steps) {
    ...
    #Make a full step for the position
    chi = chi + epsilon * p
    xi = sapply(1:num.periods, function(t) {sapply(1:num.senses, function(k){chi[,k]+theta[,t]} )}, 
                simplify = "array")
    psi = sapply(1:num.periods, function(t) {rho %*% xi[,,t] + sigma}, simplify = "array")
  ...
  out = list(MH = MH, chi = chi, psi = psi, psi.tilde = psi.tilde, sense.probs = sense.probs)
...
propose.new.theta.t.HMC = function(num.steps, epsilon, theta.t, psi.t.tilde, theta.t.prior.mean, theta.t.prior.var, 
                                   sense.probs.t, rho.times.word.counts.t, snippet.lengths.t, temper = FALSE, inv.temp = 1){
  
  #Generate initial momentum vector
  p = rnorm(embedding.dim)
  ...
  grad_U = -get.theta.t.gradient(theta.t, psi.t.tilde, theta.t.prior.mean, theta.t.prior.var, sense.probs.t, 
                                 rho.times.word.counts.t, snippet.lengths.t, temper, inv.temp)
  p = p - epsilon * grad_U / 2
  ...
  out = list(MH = MH, theta.t = theta.t, psi.t = psi.t, psi.t.tilde = psi.t.tilde, sense.probs.t = sense.probs.t)
...
#Function to propose sigma using HMC
#Function adapted from Neal, R.M., 2011. MCMC using Hamiltonian dynamics
propose.new.sigma.HMC = function(num.steps, epsilon, sigma, psi.tilde, sigma.prior.mean, sigma.prior.var, 
                                 sense.probs, word.counts.totals, snippet.lengths, temper = FALSE, inv.temp = 1){
  
  #Generate initial momentum vector
  p = rnorm(num.words)
  
  #Evaluate potential and kinetic energies at start of trajectory
  current_U = -(sum(dnorm(sigma, sigma.prior.mean, sqrt(sigma.prior.var), log = TRUE)) + 
                  ifelse(temper, inv.temp, 1) * sum(log(colSums(sense.probs))))
  current_K = sum(p^2) / 2
  
  #Make a half step for momentum at the beginning
  grad_U = -get.sigma.gradient(sigma, psi.tilde, sigma.prior.mean, sigma.prior.var, 
                               sense.probs, word.counts.totals, snippet.lengths, temper, inv.temp)
  p = p - epsilon * grad_U / 2
  ...
  for (s in 1:num.steps) {
    ...
    #Make a full step for the position
    sigma.old = sigma
    sigma = sigma + epsilon * p
    psi = sapply(1:num.periods, function(t) {psi[,,t] - sigma.old + sigma}, simplify = "array")
    psi.tilde = aperm(aaply(exp(psi), 3, function(x) t(t(x)/colSums(x))), c(2,3,1))
    
    for(t in 1:num.periods) {
      for(g in 1:num.genres) {
        sense.probs[,SnippetIDs[[t]][[g]]] = apply(snippets[SnippetIDs[[t]][[g]], 1:snippet.length], 1, 
                                                   get.sense.prob, phi.tilde[,g,t], psi.tilde[,,t])
      }#for g
    }#for t
  ...
  #Evaluate potential and kinetic energies at end of trajectory
  proposed_U = -(sum(dnorm(sigma, sigma.prior.mean, sqrt(sigma.prior.var), log = TRUE)) + 
                   ifelse(temper, inv.temp, 1) * sum(log(colSums(sense.probs))))
  proposed_K = sum(p^2) / 2
  ...
  out = list(MH = MH, sigma = sigma, psi = psi, psi.tilde = psi.tilde, sense.probs = sense.probs)
  
  return(out)
  
}#propose.new.sigma.HMC
...
rm(sense.counts, snippets.expanded, word.counts, rho.svd, mkt.combos, design.matrix.idx, design.matrix, xi, xi.fit, chi.mean); gc()
...
theta.prop.scale = array(theta.prop.scale.init, dim = c(num.periods), dimnames = list(Time = 1:num.periods))
sigma.prop.scale = sigma.prop.scale.init
...
chi.sim = array(dim = c(num.samples, embedding.dim, num.senses), 
                dimnames = list(Sample = 1:num.samples, Embedding = 1:embedding.dim, Sense = 1:num.senses))

theta.sim = array(dim = c(num.samples, embedding.dim, num.periods), 
                  dimnames = list(Sample = 1:num.samples, Embedding = 1:embedding.dim, Time = 1:num.periods))

sigma.sim = array(dim = c(num.samples, num.words), 
                  dimnames = list(Sample = 1:num.samples, Word = 1:num.words))
...
save(random.seed, num.runs, stop.tuning.after, num.iterations, N, num.samples,
     temper.phi, temper.chi, temper.theta, temper.sigma, min.inv.temp, num.iter.temp,
     kappa.phi, kappa.chi, kappa.theta, kappa.sigma, alpha.phi, alpha.theta, 
     num.leapfrog.steps.phi, num.leapfrog.steps.chi, num.leapfrog.steps.theta, num.leapfrog.steps.sigma,
     target.accept.rate.phi, target.accept.rate.chi, target.accept.rate.theta, target.accept.rate.sigma,
     phi.prop.scale.init, chi.prop.scale.init, theta.prop.scale.init, sigma.prop.scale.init,
     block.size, c1.phi, c1.chi, c1.theta, c1.sigma, #c0.phi, c0.chi, c0.theta, c0.sigma,
...
for (b in 1:num.runs) {
  ...
  for (i in 1:num.iterations) {
    ...
    chi.HMC.proposal = propose.new.chi.HMC(num.leapfrog.steps.chi, sqrt(chi.prop.scale), chi, psi.tilde, chi.prior.mean, 
                                           kappa.chi, sense.probs, rho.times.word.counts, snippet.lengths, temper.chi, inv.temp)
    ...
    for (t in time.direction) {
      ...
      theta.t.HMC.proposal = propose.new.theta.t.HMC(num.leapfrog.steps.theta, sqrt(theta.prop.scale[t]), theta[,t], 
                                                     ...
                                                     rho.times.word.counts[,SnippetIDs.by.time[[t]]],
      ...
      if(log(runif(1)) < theta.t.HMC.proposal$MH) {
        ...
        psi[,,t] = theta.t.HMC.proposal$psi.t
    ...
    sigma.HMC.proposal = propose.new.sigma.HMC(num.leapfrog.steps.sigma, sqrt(sigma.prop.scale), sigma, psi.tilde, 
                                               sigma.prior.mean, kappa.sigma, sense.probs, word.counts.totals, snippet.lengths,
                                               temper.sigma, inv.temp)
    
    #Accept or reject
    if(log(runif(1)) < sigma.HMC.proposal$MH) {
      accept.count.sigma = accept.count.sigma + 1
      sigma.jumps[i %% block.size + 1] = 1
      sense.probs = sigma.HMC.proposal$sense.probs
      sigma = sigma.HMC.proposal$sigma
      psi = sigma.HMC.proposal$psi
      psi.tilde = sigma.HMC.proposal$psi.tilde
    } else {
    ...
    if (j <= stop.tuning.after) {
      if (j >= block.size) {
        ...
        sigma.prop.scale = update.scale(sigma.prop.scale, accept.rate, gamma1, target.accept.rate.sigma)
      }#if
    }#if
  ...
  save(sigma.sim, file = paste("sigma.sim", filename.suffix, "RData", sep = "."))
  save(Start.Time, End.Time, Run.Time, file = paste("run.time", filename.suffix, "RData", sep = "."))
  save(phi.prop.scale, chi.prop.scale, theta.prop.scale, sigma.prop.scale, accept.count.phi,
       accept.count.chi, accept.count.theta, accept.count.sigma, phi.jumps, chi.jumps, theta.jumps,
       sigma.jumps, file = paste("HMC.parameters.RData", filename.suffix, "RData", sep = "."))
  ...
  if (b == num.runs) {
    rm(b, i, j, t, g, gamma1, accept.rate, chi.HMC.proposal, theta.t.HMC.proposal, theta.t.prior.mean, 
       theta.t.prior.var, phi.g.t.HMC.proposal, phi.t.prior.var, sigma.HMC.proposal); gc()
  }
