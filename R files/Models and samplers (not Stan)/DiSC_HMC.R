...
get.chi.gradient = function(chi, psi.tilde, chi.prior.mean, chi.prior.var, sense.probs, word.counts.in.snippets, snippet.lengths) {
  ...
  - (chi - chi.prior.mean) / chi.prior.var + as.matrix(word.counts.in.snippets %*% P) - I.P.psi.tilde
...
get.theta.t.gradient = function(theta.t, psi.t.tilde, theta.t.prior.mean, theta.t.prior.var, sense.probs.t, 
                                word.counts.in.snippets.t, snippet.lengths.t) {

  as.numeric(
    - (theta.t - theta.t.prior.mean) / theta.t.prior.var +
      rowSums(word.counts.in.snippets.t - 
                t(snippet.lengths.t * ((t(sense.probs.t) / colSums(sense.probs.t)) %*% t(psi.t.tilde))))
  )
...
propose.new.chi.HMC = function(num.steps, epsilon, chi, psi.tilde, chi.prior.mean, chi.prior.var, 
                               sense.probs, word.counts.in.snippets, snippet.lengths){
...
for (b in 1:num.runs) {
  ...
  for (i in 1:num.iterations) {
    ...
    chi.HMC.proposal = propose.new.chi.HMC(num.leapfrog.steps.chi, sqrt(chi.prop.scale), chi, psi.tilde, chi.prior.mean, 
                                           kappa.chi, sense.probs, word.counts.in.snippets, snippet.lengths)
