...
data {
  ...
  int<lower=1> M;  // embedding dimension
  ...
  matrix[V,M] rho;  // word embeddings
  ...
  real<lower=0> kappa_sigma;          // prior hyperparameter
}
...
parameters {
  ...
  vector[M] theta[T];  // time embedding
  vector[M] chi[K];    // sense embedding
  vector[V] sigma;     // bias or correction parameter
}
...
transformed parameters {
  ...
  for (k in 1:K) {
    for (t in 1:T) {
      psi_tilde[k,t] = softmax(rho * (theta[t] + chi[k]) + sigma);
    }
  }
...
model {
  // prior for sigma
  sigma ~ normal(0, sqrt(kappa_sigma));
